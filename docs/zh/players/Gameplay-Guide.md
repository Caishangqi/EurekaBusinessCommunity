...
手持 **商店配置器**：
1. **切换模式**：按住交互键或打开模式轮盘，选择 `商店范围选择` 模式。
2. **选择对角点**：潜行右键点击第一个方块作为起点，然后右键点击对角方块作为终点，圈定一个立体的店铺区域（AABB）。
3. **指定入口与出口**：
   - 切换到 `设置入口` 模式，右键地面方块指定顾客进入店铺的位置。
   - 切换到 `设置出口` 模式，右键地面方块指定顾客离开店铺的位置。
> [!TIP]
> 商店支持设置多个出入口。顾客寻路时会结合路径距离与当前拥堵情况智能选择最优路径。

> [!CAUTION]
> 店铺在开门营业期间或有顾客未完全离开时，禁止使用商店配置器修改边界、出入口或排队区。如需重新规划空间，请先取回钥匙打烊并等待所有顾客离店。
...
1. 在店铺内放置一个 **店铺钥匙设备**。
2. 使用商店配置器将钥匙设备与该店铺空间绑定，系统将生成一把专属的 **商店钥匙**。
3. **开始营业**：
...
1. 放置 **陈列基座** 并放入商品。
2. 安装 **价格牌模块** 并设定售价（详见 [商品陈列、标价与店铺收益](Shop-Management.md)）。
3. 放置 **收银台** 与 **收银拉杆**，并配置收银员或玩家手动结算。
