...
服务器安装可选 Retail KubeJS 适配器后，可将已注册的原版、模组或普通 KubeJS 方块声明为店铺装饰。声明的方块会阻挡店铺内 Retail 顾客的寻路，请勿把顾客通道方块声明为装饰。脚本安装、重启要求和示例见 [Retail KubeJS 集成](../addon-developers/KubeJS-Integration.md)。
